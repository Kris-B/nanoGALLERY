/**!
 * @preserve nanoGALLERY v5.2.0beta3
...
nanoGALLERY v5.2.0beta3 release notes.


##### New features
- support right-to-Left display direction (RTL)
- manage image toolbar items visibility

##### New options
- **RTL**: display direction from right to left.
  *boolean; Default: false*  
- **viewerToolbar**: new options:
    `display` (*boolean; Default: `false`*): display or hide the image toolbar  
    `btnClose` (*string; Default: `'std'`*): display close button. Possible values: `'std'` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
    `btnPrevious` (*string; Default: `'std'`*): display previous button. Possible values: `'std'` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
    `btnNext` (*string; Default: `'std'`*): display next button. Possible values: `std` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
    `btnPlay` (*string; Default: `'std'`*): display play slideshow button. Possible values: `std` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
    `btnFullscreen` (*string; Default: `'std'`*): display fullscreen button. Possible values: `std` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
    `btnMinize` (*string; Default: `'always'`*): display minimized/standard toolbar button. Possible values: `std` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
    `btnInfo` (*string; Default: `'std'`*): display info button. Possible values: `std` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
    `label` (*string; Default: `'std'`*): display image title and description. Possible values: `std` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
    `imageCounter` (*string; Default: `'std'`*): display image counter. Possible values: `std` (standard toolbar), `'min'` (minimized toolbar), `'always'`  
...
(function ($) {
  jQuery.fn.nanoGallery = function (options) {
    var settings = $.extend(true, {
      ...
      viewerToolbar : { display:true, position : 'bottom', style : 'innerImage', autoMinimize:800, btnClose:'std', btnPrevious:'std', btnNext:'std', btnPlay:'std', btnFullscreen:'std', btnMinize:'always', btnInfo:'std', label:'std', imageCounter:'std' },
...
this.thumbImgHeight = 0;           // thumbnail image height
  ...
  this.Initiate = function( element, params ) {
    ...
    // POLYFILL FOR addEventListner/removeEventListener function --> for IE8
...
//          nbCol++;
  ...
  function ResizeGalleryGrid() {
      ...
      $thumbnails.each(function() {
        ...
        if( n !== undefined ) {
          ...
          if( curCol >= maxCol ){
            ...
            curPosY+=gI[n].thumbFullHeight+gutterHeight;
          }
          cnt++;
        }
      });
    ...
    $gE.conTn.width(w+ThumbnailOuterWidth()).height(h+gI[lastN].thumbFullHeight);
...
//          item.thumbImgWidth=instance.images[0].img.naturalWidth;
  ...
  function OpenInternalViewer( imageIdx ) {
    ...
    */
    ...
    $gE.conVwTb=jQuery('<div class="toolbarContainer" style="visibility:'+(gO.viewerToolbar.display ? "visible" : "hidden")+';"><div class="toolbar"><div class="visibilityButton hideToolbarButton ngbt"></div><div class="previousButton ngbt"></div><div class="pageCounter"></div><div class="nextButton ngbt"></div><div class="playButton playPauseButton ngbt"></div>'+fs+sInfo+'<div class="closeButton ngbt"></div><div class="label"><div class="title" itemprop="name"></div><div class="description" itemprop="description"></div></div></div>').appendTo($gE.conVw);
    if( gO.viewerDisplayLogo ) {
      $gE.vwLogo=jQuery('<div class="nanoLogo"></div>').appendTo($gE.conVw);
    }
    ToolbarSetItemsVisibility();
  ...
  function ToolbarSetItemsVisibility( curMode ) {
    ...
    $gE.conVwTb.find('.closeButton').css({ display:(t.btnClose == 'always' || t.btnClose == curMode ? 'table-cell' : 'none') });
    $gE.conVwTb.find('.previousButton').css({ display:(t.btnPrevious == 'always' || t.btnPrevious == curMode ? 'table-cell' : 'none') });
    $gE.conVwTb.find('.nextButton').css({ display:(t.btnNext == 'always' || t.btnNext == curMode ? 'table-cell' : 'none') });
    $gE.conVwTb.find('.playPauseButton').css({ display:(t.btnPlay == 'always' || t.btnPlay == curMode ? 'table-cell' : 'none') });
    $gE.conVwTb.find('.fullscreenButton').css({ display:(t.btnFullscreen == 'always' || t.btnFullscreen == curMode ? 'table-cell' : 'none') });
    $gE.conVwTb.find('.visibilityButton').css({ display:(t.btnMinize == 'always' || t.btnMinize == curMode ? 'table-cell' : 'none') });
    $gE.conVwTb.find('.infoButton').css({ display:(t.btInfo == 'always' || t.btnInfo == curMode ? 'table-cell' : 'none') });
    $gE.conVwTb.find('.pageCounter').css({ display:(t.imageCounter == 'always' || t.imageCounter == curMode ? 'table-cell' : 'none') });
    $gE.conVwTb.find('.label').css({ display:(t.label == 'always' || t.label == curMode ? 'table-cell' : 'none') });
  }
...
//    $gE.conVwTb.css({'visibility':'visible'});
    ...
    if( setTxt && gO.viewerToolbar.displayLabel ) {
