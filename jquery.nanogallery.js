/**!
 * @preserve nanoGALLERY v5.3.1beta
...
##### New feature
- New API method to reload the current album (only for kind json)
  Usage: $('#nanoGallery1').nanoGallery().reload();

##### New options
- **paginationDot**: display dots for thumbnail pagination  
 ...
 */
  ...
  jQuery.nanoGallery.defaultOptions = {
    ...
    thumbnailDisplayInterval : 30, thumbnailDisplayTransition : true,
  ...
  // jQuery.fn.nanoGallery = function(options){
    ...
    if( typeof $(this).data('nanoGallery') === 'undefined'){
      ...
      // the nanoGallery if it has been attached to the object.
      // return this.data('nanoGallery');
  ...
  // global data (common to all instances of nanoGALLERY on the current html page)
  // var GLOB=null;
  ...
  this.thumbImgHeight = 0;           // thumbnail image height
      ...
      NGItems.prototype = {
        thumbSetImgHeight: function(h) {              // set thumbnail image real height for current level/resolution, and for all others level/resolutions having the same settings
          ...
          for( var i=0; i< lst.length; i++ ) {
            if( G.tn.settings.height.l1[lst[i]] == SettingsGetTnHeight() && G.tn.settings.width.l1[lst[i]] == SettingsGetTnWidth() ) {
              this.thumbs.height.l1[lst[i]]=h;
            }
          }
          for( var i=0; i< lst.length; i++ ) {
            if( G.tn.settings.height.lN[lst[i]] == SettingsGetTnHeight() && G.tn.settings.width.l1[lst[i]] == SettingsGetTnWidth() ) {
    ...
    this.Initiate = function( element, params ) {
      ...
      if( G.O.thumbnailLabel.hideIcons ) {
      ...
      if( G.O.thumbnailLabel.align == 'right' ) {
      ...
      if( G.O.thumbnailLabel.align == 'center' ) {
      ...
      if( G.O.thumbnailLabel.align == 'left' ) {
      ...
      G.$E.conTn=jQuery('<div class="nanoGalleryContainer"></div>').appendTo(G.$E.conTnParent);
      ...
      G.$E.conPagin=jQuery('<div class="nanoGalleryPagination'+(G.O.paginationDot? 'Dot':'' )+'" '+sRTL2+'></div>').appendTo(G.$E.conTnParent);
      G.$E.conPagin.hide();
      ...
      jQuery(window).resize(function() { 
        ...
        if( G.containerViewerDisplayed ) {
          ...
          resizeTimeOut = setTimeout(function () {
            ...
            if( G.curAlbumIdx != -1 && 
                  ( SettingsGetTnHeight() != G.tn.settings.height[G.curNavLevel][nw] || 
                  SettingsGetTnWidth() != G.tn.settings.width[G.curNavLevel][nw] ) ) {
      ...
      G.$E.base.on('scroll', function () {
        if(G.scrollTimeOut2) clearTimeout(G.scrollTimeOut2);
        G.scrollTimeOut2 = setTimeout(function () {
          thumbnailsLazySetSrc();
        }, 200);
      });
    ...
    function NGFinalize() {
      ...
      for( var j=0; j<G.tnHE.length; j++) {
        switch(G.tnHE[j].name ) {
          case 'imageScale150':
    ...
    function userEventsGallery(element) {
      ...
      this.handleGestureMove = function (e) {
        ...
        if( G.pgMaxLinesPerPage > 0 && SettingsGetTnHeight() != 'auto' && SettingsGetTnWidth() != 'auto' ) {
      ...
      function OpenTouchedThumbnail() {
        ...
        if( G.containerViewerDisplayed ) {
          ...
          if( G.$currentTouchedThumbnail != null ) {
            ...
            if( G.O.touchAnimation && !G.openNoDelay ) {
    ...
    function checkPluginParameters() {
      ...
      */
      ...
      switch( toType(G.O.thumbnailHoverEffect) ) {
        case 'string':
          var tmp=G.O.thumbnailHoverEffect.split(',');
          ...
          break;
        case 'object':
          if( G.O.thumbnailHoverEffect.name != 'none' && isAEngineSupported(G.O.thumbnailHoverEffect.name) ) {
            var oDef=NewTHoverEffect();
            G.tnHE.push(jQuery.extend(oDef,G.O.thumbnailHoverEffect));
          }
          break;
        case 'array':
          for(var i=0; i<G.O.thumbnailHoverEffect.length; i++) {
            if( G.O.thumbnailHoverEffect[i].name != 'none' && isAEngineSupported(G.O.thumbnailHoverEffect[i].name) ) {
              var oDef=NewTHoverEffect();
              G.tnHE.push(jQuery.extend(oDef,G.O.thumbnailHoverEffect[i]));
            }
          }
          break;
        case 'null':
    ...
    function RetrieveCurWidth() {
      ...
      if( G.O.thumbnailSizeSM > 0 && vpW < G.O.thumbnailSizeSM) { return 'xs'; }
      if( G.O.thumbnailSizeME > 0 && vpW < G.O.thumbnailSizeME) { return 'sm'; }
      if( G.O.thumbnailSizeLA > 0 && vpW < G.O.thumbnailSizeLA) { return 'me'; }
      if( G.O.thumbnailSizeXL > 0 && vpW < G.O.thumbnailSizeXL) { return 'la'; }
    ...
    function isAEngineSupported( effect ) {
      ...
      if( G.O.thumbnailLabel.position == 'onBottom' && !/borderLighter|borderDarker|imageOpacity50|imageScale150|imageScaleIn80|imageSlide2Up|imageSlide2Down|imageSlide2Left|imageSlide2Right|imageSlide2Random|imageSlide2UpRight|imageSlide2UpLeft|imageSlide2DownRight|imageSlide2DownLeft|imageScale150Outside|scale120/i.test(effect) ) {
        nanoAlert('The parameter combination thumbnailHoverEffect="'+effect+'" and thumbnailLabel.position="onBottom" is not supported.');
        return false;
      }
      
      //if( G.O.thumbnailLabel.position == 'overImageOnBottom' && /descriptionSlideUp/i.test(effect) ) {
    ...
    function GetImageTitle( imageSRC ) {
      if( G.O.thumbnailLabel.title == '%filename' ) {
        return (imageSRC.split('/').pop()).replace('_',' ');
      }
      
      if( G.O.thumbnailLabel.title == '%filenameNoExt' ) {
    ...
    function ProcessItemOption() {
      ...
      jQuery.each(G.O.items, function(i,item){
        ...
        var src=G.O.itemsBaseURL+item.src;
    ...
    function ProcessHREF(elements) {
      ...
      jQuery.each(elements, function(i,item){
        ...
        if( !(G.O.thumbnailLabel.title == '' || G.O.thumbnailLabel.title == undefined) ) {
          title=GetImageTitle(src);
        }
  ...
  //A    url = url + "&callback=?";
    ...
    function ResizeGalleryWidthtAuto() {
      ...
      $thumbnails.each(function() {
        ...
        if( n !== undefined && G.I[n] != undefined ) {
          if( G.I[n].thumbImg().width > 0 ) {
            var item=G.I[n],
            w=Math.floor(item.thumbImg().width/item.thumbImg().height*SettingsGetTnHeight())+ G.tn.borderWidth+G.tn.imgcBorderWidth; // +gutterWidth;
      ...
      $thumbnails.each(function() {
        ...
        if( n !== undefined && G.I[n] != undefined ) {
          if( G.I[n].thumbImg().width > 0 ) {
            ...
            item.thumbFullHeight=rh+G.tn.borderHeight+G.tn.imgcBorderHeight+G.tn.labelHeight;
            ...
            if( n == rowLastItem[rowNum] ) {
              // curPosY+=rowHeight[rowNum]+G.tn.outerHeight+G.tn.labelHeight+gutterHeight;
              curPosY+=rowHeight[rowNum]+G.tn.labelHeight+gutterHeight+G.tn.imgcBorderHeight+G.tn.borderHeight;
              rowNum++;
              lastPosX=0;
      ...
      tnFeaturedH=tnFeaturedH+ThumbnailOuterHeight()+G.tn.labelHeight;
    ...
    function NbThumbnailsPerRow() {
      ...
      var tnW=SettingsGetTnWidth()+G.tn.borderWidth+G.tn.imgcBorderWidth;
      var areaW=G.$E.conTnParent.width();
    ...
    function ResizeGalleryGrid() {
      ...
      G.$E.conTn.width(w+ThumbnailOuterWidth()).height(h+ThumbnailOuterHeight());
    ...
    function managePagination( albumIdx, pageNumber ) {
      ...
      var n2=0,
      w=0;
      if( !G.O.paginationDot && pageNumber > 0 ) {
        var eltPrev=jQuery('<div class="paginationPrev">'+(G.O.paginationDot? '':G.i18nTranslations.paginationPrevious)+'</div>').appendTo(G.$E.conPagin);
      ...
      for(var i=firstPage; i < n2; i++ ) {
        ...
        var elt$=jQuery('<div class="paginationItem'+c+'">'+(G.O.paginationDot? '':(i+1))+'</div>').appendTo(G.$E.conPagin);
      ...
      if( !G.O.paginationDot && (pageNumber+1) < n2 ) {
        var $eltNext=jQuery('<div class="paginationNext">'+(G.O.paginationDot? '':G.i18nTranslations.paginationNext)+'</div>').appendTo(G.$E.conPagin);
  ...
  //      G.$E.conTn.off().empty();
    ...
    function thumbnailBuild( item, idx, foundDesc ) {
      ...
      if( G.O.thumbnailLazyLoad && SettingsGetTnWidth() == 'auto' ) {
      ...
      src=G.emptyGif;
      if( (SettingsGetTnHeight() == 'auto' && G.I[idx].thumbImg().height == 0) || (SettingsGetTnWidth() == 'auto' && G.I[idx].thumbImg().width == 0) ) { checkImageSize=true; }
      ...
      if( item.kind == 'album' ) {
        // ALBUM
        if( G.O.thumbnailLabel.display == true ) {
          var imageCount='';
          if( item.contentLength > 0 ) {
            // display content counter
            switch( G.O.thumbnailLabel.itemsCount) {
              case 'title':
                ...
                sTitle +=  ' ' + G.i18nTranslations.thumbnailLabelItemsCountPart1 + '<span>' +item.contentLength + '</span>' +G.i18nTranslations.thumbnailLabelItemsCountPart2;
                break;
              case 'description':
                sDesc += ' ' + G.i18nTranslations.thumbnailLabelItemsCountPart1 + '<span>' + item.contentLength + '</span>' + G.i18nTranslations.thumbnailLabelItemsCountPart2;
                break;
              }
            }
          ...
          newElt[newEltIdx++]='<div class="labelImage" style="width:'+SettingsGetTnWidth()+'px;'+(G.O.RTL ? "direction:RTL;" :"")+'"><div class="labelFolderTitle labelTitle" >'+sTitle+'</div><div class="labelDescription" >'+sDesc+'</div></div>';
        }
      }
      else {
        // IMAGE
        if( G.O.thumbnailLabel.display == true ) {
          if( foundDesc && sDesc.length == 0 && G.O.thumbnailLabel.position == 'onBottom' ) { sDesc='&nbsp;'; }
          ...
          newElt[newEltIdx++]='<div class="labelImage" style="width:'+SettingsGetTnWidth()+'px;'+(G.O.RTL ? "direction:RTL;" :"")+'"><div class="labelImageTitle labelTitle" >'+sTitle+'</div><div class="labelDescription" >'+sDesc+'</div></div>';
        }
      }
    ...
    function getThumbnailTitle( item ) {
      ...
      if( G.O.thumbnailLabel.display == true ) {
        ...
        if( G.O.thumbnailLabel.titleMaxLength > 3 && sTitle.length > G.O.thumbnailLabel.titleMaxLength ){
          sTitle=sTitle.substring(0,G.O.thumbnailLabel.titleMaxLength)+'...';
        }
      }
    ...
    function getTumbnailDescription( item ) {
      var sDesc='';
      if( G.O.thumbnailLabel.displayDescription == true ) { 
        if( item.kind == 'album' ) {
        ...
        if( G.O.thumbnailLabel.descriptionMaxLength > 3 && sDesc.length > G.O.thumbnailLabel.descriptionMaxLength ){
          sDesc=sDesc.substring(0,G.O.thumbnailLabel.descriptionMaxLength)+'...';
        }
      }
    ...
    function thumbnailSelection($e,item,forceValue){
      ...
      if (thumbnailCheckbox.is(':checked')) {
        ...
        $e.find('.imgContainer, .labelImage').css({'background':'#000'});   //TODO --> default color
      ...
      if( SettingsGetTnHeight() == 'auto' ) {
        ...
        if( item.thumbImg().height > 0 ) {
          ...
          if( G.O.thumbnailLabel.position == 'onBottom' ) {
            ...
            item.thumbFullHeight=SettingsGetTnWidth()*ratio + item.thumbLabelHeight + G.tn.borderHeight+G.tn.imgcBorderHeight;
            $elt.width(ThumbnailOuterWidth()-G.tn.borderWidth).height(item.thumbFullHeight-G.tn.borderHeight);
        ...
        $elt.find('.subcontainer').width(ThumbnailOuterWidth()-G.tn.borderWidth).height(item.thumbFullHeight-G.tn.borderHeight); //.css({'overflow':'hidden'});
      }
      else
        ...
        if( SettingsGetTnWidth() == 'auto' ) {
          ...
          if( item.thumbImg().width > 0 ) {
            ...
            if( G.O.thumbnailLabel.position == 'onBottom' ) {
              item.thumbFullWidth=SettingsGetTnHeight()*ratio + G.tn.borderWidth+G.tn.imgcBorderWidth ;
              $elt.width(item.thumbFullWidth).height(ThumbnailOuterHeight()+G.tn.labelHeight-ThumbnailOuterHeight());
            }
            else {
            }
          }
          item.thumbFullHeight=ThumbnailOuterHeight()+G.tn.labelHeight;
          $elt.find('.subcontainer').width(item.thumbFullWidth-G.tn.borderWidth).height(ThumbnailOuterHeight()-G.tn.borderHeight); //.css({'overflow':'hidden'});
        }
        
        else {
          // GRID LAYOUT
          item.thumbFullHeight=ThumbnailOuterHeight();  //+G.tn.labelHeight;  //G.tn.defaultFullHeight;
          item.thumbFullWidth=ThumbnailOuterWidth();   //G.tn.defaultFullWidth;
          if( G.O.thumbnailLabel.position == 'onBottom' ) {
    ...
    function thumbnailPositionContent( $e, item ) {
      ...
      if( !G.O.thumbnailLabel.display ) { return; }
  ...
  //    if( n == undefined ) { return; }
  //    
  //    if( G.O.touchAnimation == false || G.I[n].hovered === true ) {
  //      OpenThumbnail(n);
  //    }
  //    else {
  ...
  //  }
    ...
    function ThumbnailInit( $e ) {
      ...
      for( j=0; j<G.tnHE.length; j++) {
        switch( G.tnHE[j].name ) {
          ...
          case 'imageSlide2DownLeft':
            $e.css({overflow:'hidden'});
            item.customData.hoverEffectRDir=G.tnHE[j].name;
    ...
    function TnAni( $e, n, anime, item, eltClass) {
      ...
      if( G.aengine == 'animate' ) {
        ...
        var tf=transform[i];
          if( anime[tf] !== undefined ) {
            ...
            if( G.tnHE[n].delay > 0 ) {
              jQuery(from).delay(G.tnHE[n].delay).animate(to, { duration:G.tnHE[n].duration, easing:G.tnHE[n].easing, queue:false, step: function(currentValue) {
              ...
              jQuery(from).animate(to, { duration:G.tnHE[n].duration, easing:G.tnHE[n].easing, queue:false, step: function(currentValue) {
      ...
      //$e.find('.imgContainer').eq(0).delay(G.tnHE[n].delay)[G.aengine]({'right':'50%', 'top':'-50%'},G.tnHE[n].duration, G.tnHE[n].easing);
      if( G.tnHE[n].delay > 0 ) {
        // $e.delay(G.tnHE[n].delay)[G.aengine](anime, G.tnHE[n].duration, G.tnHE[n].easing);
        if( G.aengine == 'transition' ) {
          // transit has a bug on queue --> we do not use it
          $e.delay(G.tnHE[n].delay)[G.aengine](anime, G.tnHE[n].duration , G.tnHE[n].easing );
        }
        else {
          $e.delay(G.tnHE[n].delay)[G.aengine](anime, {duration:G.tnHE[n].duration , easing:G.tnHE[n].easing, queue:false });
        }
      }
      else {
        // $e[G.aengine](anime, G.tnHE[n].duration, G.tnHE[n].easing);
        if( G.aengine == 'transition' ) {
          // transit has a bug on queue --> we do not use it
          //anime.queue=false;
          //anime.duration=5000;
          ...
          $e[G.aengine](anime, {duration:G.tnHE[n].duration , easing:G.tnHE[n].easing, queue:false});
        }
      }
    }
    ...
    function TnAniO( $e, n, anime, item, eltClass) {
      ...
      if( G.aengine == 'animate' ) {
        for( var i=0; i<transform.length; i++ ) {
          ...
          if( anime[tf] !== undefined ) {
            var from = {v: parseInt(item.eltTransform[eltClass][tf]), item:item, tf:tf, eltClass:eltClass, to:parseInt(anime[tf]) };
            var to = {v: parseInt(anime[tf])};
            if( G.tnHE[n].delayBack > 0 ) {
              jQuery(from).delay(G.tnHE[n].delayBack).animate(to, { duration:G.tnHE[n].durationBack, easing:G.tnHE[n].easingBack, queue:false, step: function(currentValue) {
                  item.eltTransform[this.eltClass][this.tf]=currentValue;
                  SetCSSTransform(this.item, this.eltClass);
              ...
              jQuery(from).animate(to, { duration:G.tnHE[n].durationBack, easing:G.tnHE[n].easingBack, queue:false, step: function(currentValue) {
                item.eltTransform[this.eltClass][this.tf]=currentValue;
                SetCSSTransform(this.item, this.eltClass);
                // var i = Math.round(this.v);
                // this.curr || (this.curr = i);
      ...
      //$e.find('.imgContainer').eq(0).delay(G.tnHE[j].delay)[G.aengine]({'right':'50%', 'top':'-50%'},G.tnHE[j].duration, G.tnHE[j].easing);
      if( G.tnHE[n].delay > 0 ) {
        if( G.aengine == 'transition' ) {
          // transit has a bug on queue --> we do not use it
          $e.delay(G.tnHE[n].delayBack)[G.aengine](anime, G.tnHE[n].durationBack, G.tnHE[n].easingBack);
        }
        else {
          $e.delay(G.tnHE[n].delayBack)[G.aengine](anime, {duration:G.tnHE[n].durationBack , easing:G.tnHE[n].easingBack, queue:false });
        }
      }
      else {
        if( G.aengine == 'transition' ) {
          // transit has a bug on queue --> we do not use it
          $e[G.aengine](anime, G.tnHE[n].durationBack, G.tnHE[n].easingBack);
        }
        else {
          $e[G.aengine](anime, {duration:G.tnHE[n].durationBack , easing:G.tnHE[n].easingBack, queue:false});
        }
      }
    }
    ...
    function OpenInternalViewer( imageIdx ) {
      ...
      sImg+='<img class="image" src="'+G.I[imageIdx].responsiveURL()+'" alt=" " style="visibility:visible;opacity:0;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;zoom:1;" itemprop="contentURL">';
      sImg+='<img class="image" src="'+G.I[imageIdx].responsiveURL()+'" alt=" " style="visibility:visible;opacity:0;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;zoom:1;" itemprop="contentURL">';
      sImg+='<img class="image" src="'+G.I[imageIdx].responsiveURL()+'" alt=" " style="visibility:visible;opacity:0;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto;zoom:1;" itemprop="contentURL">';

      G.$E.vwContent=jQuery('<div class="content">'+sImg+'<div class="contentAreaPrevious"></div><div class="contentAreaNext"></div></div>').appendTo(G.$E.conVw);
      ...
      */
      ...
      var $closeB=jQuery('<div class="closeButtonFloating"></div>').appendTo(G.$E.conVw);
      ...
      // build image toolbar
      var sTB='<div class="toolbarContainer" style="visibility:'+(G.O.viewerToolbar.display ? "visible" : "hidden")+';"><div class="toolbar">';
      ...
      switch( e ) {
        ...
        default:
          if( e.indexOf('custom') == 0 ) {
            r='<div class="ngbt ngCustomBtn '+e+'">' + (typeof G.O.fnImgToolbarCustInit == 'function' ? G.O.fnImgToolbarCustInit(e) : '') + '</div>';
          }
          break;
      }
      return r;
    ...
    function ViewerFullscreenToggleOLD(){
      if( G.viewerIsFullscreen ) {
        ...
        if (G.$E.conVwCon[0].requestFullscreen) {
                    G.$E.conVwCon[0].requestFullscreen();
                } else if (G.$E.conVwCon[0].webkitRequestFullscreen) {
                  G.$E.conVwCon[0].webkitRequestFullscreen();
                } else if (G.$E.conVwCon[0].msRequestFullscreen) {
                  G.$E.conVwCon[0].msRequestFullscreen();
                } else if (G.$E.conVwCon[0].mozRequestFullScreen) {
                  G.$E.conVwCon[0].mozRequestFullScreen();
                }
        G.viewerIsFullscreen=true;
        G.$E.conVwTb.find('.fullscreenButton').removeClass('setFullscreenButton').addClass('removeFullscreenButton');
      }
    ...
    function DisplayInternalViewer( imageIdx, displayType ) {
      ...
      ResizeInternalViewer();
      window.cancelAnimationFrame(G.viewerResizeTimerID);
    ...
    function ResizeInternalViewer() {
      ...
      G.viewerResizeTimerID=window.requestAnimationFrame( ResizeInternalViewer );
